	...
	public static void chamarTela(String endereco, String user, Class<?> controllerClass) {
	    ...
	    try {
	        ...
	        if (controllerClass.equals(MenuUsuarioController.class)) {
	            ((MenuUsuarioController) controller).lblUser.setText(user); //cast do objeto controller para o tipo MenuUsuarioController para acessar o atributo lblUser
	        } else if (controllerClass.equals(MenuAdministradorController.class)) {
	            ((MenuAdministradorController) controller).getLblUser().setText(user);
	        }
